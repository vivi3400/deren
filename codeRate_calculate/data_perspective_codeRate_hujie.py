...
def get_pivottable(data,start,end):

	# 获取前120s的数据 其余的去掉
	data_ = data[(data['ip.proto']==int(args))&(data['frame.time_relative']<=end)&(data['frame.time_relative']>=start)]
	...
	print('------------------------------------')
	print('CodeRate in %ss in %s protocol with start %s s and end %s s'%(duration,args,start,end))
	print('------------------------------------')
