...
# 获取前120s的数据 其余的去掉
# data_ = data[(data['ip.proto']==args) & (data['frame.time_relative']<=duration)]

data_ = data[(data['ip.proto']==int(args)) & (data['frame.time_relative']<=duration)]



# 计算码率 将每一个value都用码率公式计算一次
data_[u'frame.len'] = data_[u'frame.len'].map(lambda x: x*8/duration/1000)
